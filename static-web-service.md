...
<img src="fig/static-web-service-stack.png" width="70%" />
...
### 6.1. 윈도우 환경 소프트웨어 카펜트리 툴체인 설치

윈도우 환경에서 소프트웨어 카펜트리 툴체인 설치을 설치할 경우 소프트웨어 카펜트리에서 직접 개발된 윈도우 설치자(Windows Installer)를 다운로드 받아 설치한다. 

윈도우 설치자는 [GitHub windows-installer](https://github.com/swcarpentry/windows-installer/releases/tag/v0.3)를 다운로드하면 된다.

### 6.2. 파이썬 한글 인코딩 관련 문제 해결

파이콘 KR 2015에서 파이썬 문자 인코딩관련 내용이 잘 발표되었다.[^1]
인코딩 관련하여 다음과 같은 유니코드 오류가 생길 경우 다음과 같이 대처한다.

~~~ {.error}
UnicodeDecodeError: ‘ascii’ codec can’t decode byte 0xeb in position 0: ordinal not in range(128)
~~~

윈도우 사용자의 경우 `C:\Python27\Lib\site-packages`에 `sitecustomize.py` 파일을 새로 만들고, 다음 내용을 적어 넣는다. 즉, 디폴트 기본설정 인코딩으로 `utf-8`으로 설정한다.[^2]
...
### 6.3. 소프트웨어 카펜트리 필터 오류[^3]

`Error running filter tools/filters/blockquote2div.py` 오류가 발생할 경우 **pandocfilters**가 설치되지 않아 발생되는 문제인만큼 `pip install pandocfilters` 명령어로 `pandocfilters`를 설치한다.
...
pandoc -s -t html \
            ...
            -o 100-cad.html 100-cad.md
pandoc.exe: Error running filter tools/filters/blockquote2div.py
terminateProcess: permission denied (Permission denied)
make: *** [100-cad.html] Error 83
~~~
...
[^1]: [Character Encoding in python](http://www.slideshare.net/daesung7kang/character-encoding-in-python)
[^2]: [python 한글 문제](http://cafecola.tistory.com/11)
[^3]: [Error running filter tools/filters/blockquote2div.py](https://github.com/swcarpentry/lesson-template/issues/107)